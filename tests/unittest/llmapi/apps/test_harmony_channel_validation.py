...
try:
    ...
    _check_channel_valid = getattr(harmony_adapter, '_check_channel_valid',
                                   None)
...
class TestHarmonyStreamStateChannelValidation:
    ...
    def test_create_openai_streaming_response_with_should_stop_signal(
            self, harmony_adapter):
        ...
        with patch.object(harmony_adapter,
            ...
            response_data = json.loads(responses[0].replace("data: ",
                                                            "").strip())
            assert response_data["choices"][0]["finish_reason"] == "stop"

    def test_create_openai_streaming_response_without_stop_signal(
            self, harmony_adapter):
        ...
        with patch.object(harmony_adapter,
            ...
            for response_str in responses:
                response_data = json.loads(
                    response_str.replace("data: ", "").strip())
...
class TestHandleStreamingResponse:
    ...
    def test_handle_streaming_response_aborts_on_invalid_channels(
            self, mock_result):
        """
        Test that handle_streaming_response() calls result.abort() when
        invalid channel sequence is detected.
        ...
        from tensorrt_llm.serve.harmony_adapter import (
            get_harmony_adapter, handle_streaming_response)
        ...
        harmony_adapter.create_stream_state(request_id=request_id,
                                            available_tools=None,
                                            tool_choice=None)
        ...
        try:
            ...
            with patch.object(
                    harmony_adapter,
                    'create_openai_streaming_response') as mock_create_response:
    ...
    def test_handle_streaming_response_continues_on_valid_channels(
            self, mock_result):
        """
        Test that handle_streaming_response() does NOT abort when
        valid channel sequences are processed.
        ...
        try:
            ...
            with patch.object(
                ...
                # Call handle_streaming_response
                responses = handle_streaming_response(tools=[],
                                                      tool_choice=None,
                                                      result=mock_result,
                                                      model="test-model",
                                                      request_id=request_id,
